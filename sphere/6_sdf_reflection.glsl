...
vec3 shading(vec4 scene, vec3 p, vec3 n, vec3 lpos, vec3 rdir)
{
    vec3 clr = m_color(scene.y, p);
    float spec = specular(p, rdir, n, lpos, 55.);

    float a = albedo(p, n, lpos);
    clr *= max(0.25, a);
    clr += spec * vec3(1.0, 1.0, 1.0)*(10.0/length(lpos-p));

    float sh = shadow(p, n, lpos);

    // clr += vec3(1., 1., 1.) * spec * sh;
    return clr;
}
...
void main() {
    ...
    // shading
    vec3 color = shading(scene, rpos, n, lpos, rdir);

    float spec = specular(rpos, rdir, n, lpos, 99.);
    ...
    if (scene.y == 2.0) 
    {
        ...
        reflected_color = shading(scene, reflected_pos, normal(reflected_pos), lpos, reflected_dir);
    ...
    // shitty hack to add specular post reflections
    color += spec * vec3(1.0, 1.0, 1.0)*(10.0/length(lpos-rpos));

    color *= max(0.5, shadow(rpos, n, lpos));
